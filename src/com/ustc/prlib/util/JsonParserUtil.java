...
public class JsonParserUtil {
	...
	/**
	 * 将单层的json数据转换为对象 { "":"" } 该对象必须继承自:BaseVo
	 */
	public static BaseVo parseJson2Vo(InputStream in,
			Class<? extends BaseVo> classzz) {
		BaseVo vo = null;
		try {
			Gson gson = new Gson();
	...
	public static BaseVo parseJson2Vo(String jsonStr,
			Class<? extends BaseVo> classzz) {
		BaseVo vo = null;
		try {
			Gson gson = new Gson();
	 ...
	 * @return
	 */
	public static BaseVo JsonElement2Obj(JsonElement element,
			Class<? extends BaseVo> classzz) {
		Gson gson = new Gson();
	...
	/**
	 * 将数据转换为数组,json格式 (包含item根节点) { "item":[ { "text":"" } , { "text":"" } ] }
	 * 
	 * @param jsonStr
	 *            json字符串
	 * @param nodeName
	 *            item节点名字
	 * @param type
	 *            new TypeToken<BaseVo>(){}.getType();
	 * @return
	 */
	public static ArrayList<? extends BaseVo> parseJson2List(String jsonStr,
			Type type, String nodeName) {
	...
	/**
	 * 将数据转换为数组,json格式 (不包含item根节点) [ { "text":"" } , { "text":"" } ]
	 * 
	 * @param jsonStr
	 * @return
	 */
	public static ArrayList<? extends BaseVo> parseJson2ListNoItem(
			String jsonStr, Type type) {
		ArrayList<? extends BaseVo> arrvo = null;
		try {
			Gson gson = new Gson();
